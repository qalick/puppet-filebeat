...
class filebeat::install::windows {
  ...
  # perform different actions if absent
  if $filebeat::package_ensure != 'absent' {
    ...
    exec { "remove service ${filename}":
      cwd     => $install_folder,
      command => './uninstall-service-filebeat.ps1',
      onlyif  => "Test-Path -Path ${install_folder}",
    }

    exec { "remove install_folder ${filename}":
      cwd     => $filebeat::install_dir,
      command => "Remove-Item '${install_folder}' -Recurse -Force -ErrorAction SilentlyContinue",
      onlyif  => "Test-Path -Path ${install_folder}",
    }
  }
}
